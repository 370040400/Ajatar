...
def configFileParser(configFile):
    ...
    for option in ("threadNum","crawlerDeep","TimeOut","UserAgent","Cookie","headers"):
    ...
    for family, optionData in optDict.items():#返回可遍历的(键, 值) 元组数组
        for option, datatype in optionData.items():
            datatype = unArrayizeValue(datatype)  #判断是否为元组，列表并处理后返回
            #family=config  option=threadNum....  datatype=integer....
